...
export default function Contact({ data, siteSettings, userId, user }: { data: any; siteSettings?: any; userId?: string; user?: any }) {
  const [form, setForm] = useState({ name: "", email: "", subject: "", message: "" });
  const [status, setStatus] = useState<"idle" | "sending" | "sent" | "error">("idle");
  const sectionRef = useRef<HTMLDivElement>(null);

  // Priority for email/phone: user account > siteSettings override > aboutContent
  // (user account is the most reliable source for who this portfolio belongs to)
  const resolvedEmail    = user?.email    || siteSettings?.contactEmail    || data?.email    || "";
  const resolvedPhone    = user?.phone    || siteSettings?.contactPhone    || data?.phone    || "";
  const resolvedLocation = siteSettings?.contactLocation || data?.location || "";
  const resolvedAvail    = siteSettings?.contactAvailability || data?.availability || "";

  const infoItems = [
    { icon: "📍", label: "Location",     value: resolvedLocation },
    { icon: "📧", label: "Email",        value: resolvedEmail },
    { icon: "📱", label: "Phone",        value: resolvedPhone },
    { icon: "🕐", label: "Availability", value: resolvedAvail },
  ].filter((item) => item.value);

  const contactSubtitle = siteSettings?.contactSubtitle || "";
