...
const socials = [
  ...
  { label: "Email", href: "mailto:nithish@example.com", icon: (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
  ), color: "#ec4899" },
];

export default function Contact({ data, userId }: { data: any; userId?: string }) {
  const [form, setForm] = useState({ name: "", email: "", subject: "", message: "" });
  const [status, setStatus] = useState<"idle" | "sending" | "sent" | "error">("idle");
  const sectionRef = useRef<HTMLDivElement>(null);

  const infoItems = [
    { icon: "📍", label: "Location", value: data?.location || "Chennai, Tamil Nadu, India" },
    { icon: "📧", label: "Email", value: data?.email || "nithish@example.com" },
    { icon: "📱", label: "Phone", value: data?.phone || "+91 98765 43210" },
    { icon: "🕐", label: "Availability", value: data?.availability || "Open to opportunities" },
  ];
          ...
          <div className={`${styles.infoCol} reveal`}>
            <div className={`glass-card ${styles.infoCard}`}>
              ...
              <p className={styles.infoSubtitle}>
                Whether you need a full-stack web application, an AI integration, or a technical consultation, my inbox is always open. Let&apos;s create something impactful.
              </p>
